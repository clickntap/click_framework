...
public class JsonUtils
{
  ...
  public static Bean jsonToBean(String jsonAsString) throws Exception {
    ...
    Bean bean = (Bean)Class.forName(json.getString("class")).newInstance();
    for (Object key : json.keySet()) {
      if (key.equals("class"))
        continue; 
      JSONObject field = json.getJSONObject(key.toString());
      if (field.getString("type").equals("datetime")) {
        BeanUtils.setValue(bean, key.toString(), new Datetime(field.getString("value"))); continue;
      }  if (field.getString("type").equals("number")) {
        BeanUtils.setValue(bean, key.toString(), field.get("value")); continue;
      } 
